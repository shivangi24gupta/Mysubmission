...
def index():
    ...
    for x in row:
        x["total"]=usd(x["price"]*x["quantity"])
        x["price"]=usd(x["price"] )  
        
    if t[0]["sum(price*quantity)"] is None:
        tot=0
    else:
        tot=t[0]["sum(price*quantity)"]+ucash[0]["cash"]
    return render_template("index.html",cash=usd(ucash[0]["cash"]),stock=row,total=usd(tot),status=request.args.get('status'))
...
def buy():
    if request.method == "POST":
        ...
        if not request.form.get("quantity"):
           ...
           return redirect(url_for("index",status="bought"))
    else:  
...
def addCash():
    if request.method == "POST":
        ...
        return redirect(url_for("index", status="cash is added"))
...
def changePassword():
    if request.method == "POST":
        ...
        return redirect(url_for("index",status="password is changed"))
...
def login():
    ...
    if request.method == "POST":
        ...
        # redirect user to home page
        return redirect(url_for("index",status="loged in"))
...
def sell():
    if request.method == "POST":
        ...
        if len(row)==0 or row[0]["quantity"]<n : 
           ...
           return redirect(url_for("index",status="sold"))
    else:
